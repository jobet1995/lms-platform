...
const TECH_ARTICLES = ARTICLES.filter(a => a.category === 'Tech');
        ...
        <div className="grid gap-6 sm:grid-cols-2">
          {TECH_ARTICLES.map((a) => (
            <Link key={a.id} href={`/blog/${a.id}`} className="group rounded-2xl border border-base-content/8 bg-base-100 overflow-hidden hover:shadow-xl transition-all flex flex-col">
              <Image src={a.image} alt={a.title} width={800} height={450} className="h-52 w-full object-cover" />
              <div className="p-6 flex flex-col gap-3 flex-1">
                ...
                <div className="flex items-center justify-between mt-2">
                  <div className="flex items-center gap-2">
                    <Image src={a.author.avatar} alt={a.author.name} width={28} height={28} className="w-7 h-7 rounded-full object-cover" />
